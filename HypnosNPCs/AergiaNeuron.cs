...
namespace Hypnos.HypnosNPCs
{
    internal class AergiaNeuron : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            switch (NPC.ai[0])
            {
                ...
                case 7: //Rings
                    {
                        ...
                        if (CalamityWorld.revenge)
                        {
                            switch (NPC.ai[1])
                            {
                                ...
                                case 8:
                                    NPC.ai[3] = 400;
                                    lasertimer = hypnos.ModNPC<HypnosBoss>().ragetimer > 0 ? 40 : 50;
                                    break;
                                case 9:
                                case 10:
                                case 11:
                                    NPC.ai[3] = 500;
                                    lasertimer = hypnos.ModNPC<HypnosBoss>().ragetimer > 0 ? 25 : 30;
                                    break;
                            }
                        }
                        else
                ...
                case 8: //Predictive charge
                    {
                        NPC.velocity = Vector2.Zero;
                        rottimer += hypnos.ModNPC<HypnosBoss>().ragetimer > 0 ? 15f : 12f;
                        double deg = 30 * NPC.ai[1] + rottimer;
                        double rad = deg * (Math.PI / 180);
                        double dist = 200;
                        if (CalamityWorld.revenge)
        ...
        public void doafterimages(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Color neurcolor = hypnos.ModNPC<HypnosBoss>().ragetimer > 0 ? Color.Red : drawColor;
